...
# Calculate date - use 5 years ago to get more history (or remove limit entirely)
if [[ "$OSTYPE" == "darwin"* ]]; then
    SINCE_DATE=$(date -u -v-5y +"%Y-%m-%dT%H:%M:%SZ" 2>/dev/null || date -u -j -f "%Y-%m-%d" "$(date +%Y-%m-%d)" -v-5y +"%Y-%m-%dT%H:%M:%SZ")
else
    SINCE_DATE=$(date -u -d "5 years ago" +"%Y-%m-%dT%H:%M:%SZ")
fi
...
REPOS=$($GH_CMD api user/repos --paginate --jq '.[] | .full_name')
...
while IFS= read -r REPO_FULL_NAME; do
    ...
    if [ -n "$COMMITS" ]; then
        ...
        # Insert/update commits and fetch stats for each
        ...
        for (const c of commits) {
          try {
            // Check if already exists and has stats
            const existing = db.prepare('SELECT id, additions, deletions FROM github_commits WHERE sha = ?').get(c.sha);
            const needsStats = !existing || (existing.additions === 0 && existing.deletions === 0);
            
            // Fetch detailed commit stats if needed
            let additions = 0;
            let deletions = 0;
            let filesChanged = 0;
            ...
            if (existing) {
              // Update existing commit with stats
              if (needsStats) {
                db.prepare(\`
                  UPDATE github_commits 
                  SET additions = ?, deletions = ?, files_changed = ?
                  WHERE sha = ?
                \`).run(additions, deletions, filesChanged, c.sha);
                updated++;
              }
            } else {
              // Insert new commit
        ...
        db.close();
        ")
        NEW_COMMITS=$(echo "$INSERTED" | cut -d'|' -f1)
        UPDATED_COMMITS=$(echo "$INSERTED" | cut -d'|' -f2)
        TOTAL_COMMITS=$((TOTAL_COMMITS + NEW_COMMITS))
        echo "      Added: $NEW_COMMITS, Updated stats: $UPDATED_COMMITS"
    fi
