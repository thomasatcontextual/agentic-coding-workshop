...
const seasonConfig: Record<string, { icon: typeof Snowflake; color: string; bgGradient: string }> = {
  Winter: { icon: Snowflake, color: "text-blue-500", bgGradient: "from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20" },
  Spring: { icon: Leaf, color: "text-green-500", bgGradient: "from-green-50 to-emerald-50 dark:from-green-950/20 dark:to-emerald-950/20" },
  Summer: { icon: SunIcon, color: "text-yellow-500", bgGradient: "from-yellow-50 to-orange-50 dark:from-yellow-950/20 dark:to-orange-950/20" },
  Fall: { icon: Leaf, color: "text-orange-500", bgGradient: "from-orange-50 to-amber-50 dark:from-orange-950/20 dark:to-amber-950/20" },
};
...
function CorrelationCard({ 
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border-2 p-6 bg-gradient-to-br ${color} transition-all hover:scale-105 hover:shadow-lg`}>
      <div className="flex items-start justify-between mb-4">
        <Icon className={`h-8 w-8 ${color.includes('blue') ? 'text-blue-600' : color.includes('green') ? 'text-green-600' : 'text-yellow-600'}`} />
...
function SeasonCard({ season, stats }: { season: string; stats: SeasonalStats }) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border-2 p-6 bg-gradient-to-br ${config.bgGradient} transition-all hover:scale-105 hover:shadow-lg`}>
  ...
  useEffect(() => {
    ...
    const autoSync = async () => {
      ...
      try {
        ...
        if (currentStats.commits.total_commits === 0) {
        ...
        } else if (currentStats.commits.total_commits > 0 && currentStats.weather.weather_days === 0) {
          console.log("Auto-syncing weather data...");
          await syncWeather();
        } else {
          await loadData();
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <CorrelationCard
                  ...
                  color="from-blue-50 to-cyan-50 dark:from-blue-950/20 dark:to-cyan-950/20 border-blue-200 dark:border-blue-800"
                />
                <CorrelationCard
                  icon={Droplets}
                  label="Precipitation"
                ...
                <CorrelationCard
                  ...
                  color="from-yellow-50 to-orange-50 dark:from-yellow-950/20 dark:to-orange-950/20 border-yellow-200 dark:border-yellow-800"
                />
              </div>
            </CardContent>
          </Card>
