  ...
  const loadData = async () => {
    ...
    try {
      const [statsRes, analysisRes] = await Promise.all([
        ...
        fetch("/api/analysis/correlations").catch(() => ({ ok: false, json: () => ({}) })),
      ]);

      const statsData = await statsRes.json();
      let analysisData = { seasonalStats: [], commitsByTemp: [], commitsByPrecip: [], correlations: null };
      
      if (analysisRes.ok) {
        try {
          analysisData = await analysisRes.json();
        } catch (e) {
          console.warn("Failed to parse correlations data:", e);
        }
      } else {
        console.warn("Correlations API failed, continuing without it");
      }
